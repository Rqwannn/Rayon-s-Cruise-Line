...
$product = mysqli_query($conn, "SELECT * FROM data_pemesanan WHERE no_pemesanan LIKE '%$keyword%' OR nama_pemesan LIKE '%$keyword%' OR total LIKE '%$keyword%'");
            ...
            <?php $i = 1; ?>
                <?php foreach( $product as $result) : ?>
                    <tr>
                        ...
                        <td class="<?php echo ($result["confirmed"] == 1) ? "status-pembayaran-true" : "status-pembayaran-false"; ?>"><?php echo ($result["confirmed"] == 1) ? "Lunas" : "Belum Lunas"; ?></td>
                    </tr>
                <?php $i++; ?>
            <?php endforeach; ?>
        </table>
