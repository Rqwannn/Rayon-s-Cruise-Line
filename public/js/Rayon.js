...
const hitungMundur = setInterval(function (){
    ...
    teks.innerHTML = 'Batas Penawaran Tinggal <br>'
...
function validasi(e){
    ...
    if(tiket.value.length > 0 && berangkat.value.length > 1 && tujuan.value.length > 1 && tanggal.value.length > 1){
        ...
        e.preventDefault();
        $.ajax({
            url: "http://localhost/MVC-KapalLaut/public/js/json.php",
            type: 'POST',
            dataType: 'JSON',
            data: {
                'tiket' : $('#tiket').val(),
        ...
        text.innerHTML = "Data Harus Di Isi Dengan Benar";
        text.style.display = "block";
        text.style.color = "red";
        return false;
        }
    }
        var info = document.querySelector(".section-info-tiket");
