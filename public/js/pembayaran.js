    ...
    totalLayanan.forEach((resultLayanan) => {
      totalKamar.forEach((resultKamar) => {
        PembungkusHarga.forEach((resultHarga) => {
          ...
          let PenjumlahanHarga =
            parseInt(resultTiket.innerHTML) +
            parseInt(resultLayanan.innerHTML) +
            parseInt(resultKamar.innerHTML);
...
let PenjumlahanHarga =
  parseInt(totalTiket.innerHTML) +
  parseInt(totalLayanan.innerHTML) +
  parseInt(totalKamar.innerHTML);
...
function setTimePembayaran() {
  ...
  const tanggat = new Date(
    `${arrbulan[bulan]} ${awal.getDate()}, ${awal.getFullYear()} ${
      awal.getHours() + 3
    }:0:0`
  ).getTime();
...
inputValue2.addEventListener("keyup", function () {
  if (
    inputValue2.value.toLowerCase() == "visa" ||
    inputValue2.value.toLowerCase() == "master card" ||
    inputValue2.value.toLowerCase() == "american express"
  ) {
